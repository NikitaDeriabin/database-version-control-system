...
public sealed class BranchService : BaseService, IBranchService
{
    ...
    public BranchService(DbGuardCoreContext context, IMapper mapper, IUserIdGetter userIdGetter, IUserService userService) : base(context, mapper)
    {
    ...
    public async Task<(BranchCommit?, bool)> FindHeadBranchCommitAsync(Branch branch)
    {
        ...
        {
            var headBranchCommit = currentBranch.BranchCommits.LastOrDefault(x => x.IsHead);
    ...
    public async Task<int> GetLastBranchCommitIdAsync(int branchId)
    {
        ...
        return (await FindHeadBranchCommitAsync(branch)).Item1?.CommitId ?? default;
    }
